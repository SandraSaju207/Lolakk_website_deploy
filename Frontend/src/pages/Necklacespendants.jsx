...
import { ShoppingBag } from "lucide-react"; // ✅ added (you were using it)
...
export default function NecklacesPendants() {
  const [loading, setLoading] = useState(true);
  const [filters, setFilters] = useState(INITIAL_FILTERS);
  const [sort, setSort] = useState("latest");

  // ✅ FIX ADDED STATES (missing)
...
const [showFilters, setShowFilters] = useState(false);
  ...
  const fetchProducts = async () => {
    try {
      ...
      // filter necklaces + pendants from backend
...
const [products, setProducts] = useState([]);
  ...
  return (
    (filters.category === "all" ||
      item.materialType?.toLowerCase() === filters.category) &&
...
<div>
  ...
  {["all", "traditional", "modern", "casual"].map((item) => (
    <button
      ...
      className={`block text-sm mb-2 transition-all duration-200 ${
        filters.style === item
          ? "text-amber-400 font-bold drop-shadow-[0_0_8px_rgba(245,158,11,0.6)]"
          : "text-gray-400 hover:text-amber-400"
      }`}
    >
      {item}
  ...
  {isFiltered && (
    <button
      onClick={clearAllFilters}
      className="flex items-center gap-2 text-[10px] uppercase tracking-widest text-red-400/80 hover:text-red-400 transition-all border-b border-red-400/20 pb-1"
    >
      <svg
  ...
  <div className="flex flex-col flex-1">
    ...
    <div className="mt-auto pt-4">
     ...
     {showModal && selectedProduct && (
      ...
      {/* NOTIFICATION FIXED ABOVE MODAL */}
  {notification && (
